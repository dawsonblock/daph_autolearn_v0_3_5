...
class MathWorkingState(BaseModel):
    ...
    def enforce_no_answer(self)->"MathWorkingState":
        if self.answer not in (None,""): raise ValueError("Final answers are forbidden in process-only working state")
        return self
...
class MathExample(BaseModel):
    example_id:str
    split:Literal["train","iid","composition","ood","operand_ood","structural_ood","operator_ood","distractor_ood","counterfactual","adversarial"]
    question:str
    answer:str
    state:MathWorkingState
    template:str
    difficulty:int
    # v0.5.2: skill label for routing supervision
